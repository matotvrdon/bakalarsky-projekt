...
public class MappingProfile : Profile
{
    public MappingProfile()
    {
        ...
        CreateMap<ParticipantStatusAssignment, ParticipantStatusAssignmentDto>()
            .ForMember(
                dest => dest.StatusName,
                opt => opt.MapFrom(src => src.ParticipantStatus.Name)
            )
            .ForMember(
                dest => dest.RequiresApproval,
                opt => opt.MapFrom(src => src.ParticipantStatus.RequiresApproval)
            )
            .ForMember(
                dest => dest.OriginalFileName,
                opt => opt.MapFrom(src => src.FileManager != null
                    ? src.FileManager.OriginalFileName
                    : null)
            )
            .ForMember(
                dest => dest.FileName,
                opt => opt.MapFrom(src => src.FileManager != null
                    ? src.FileManager.FileName
                    : null)
            )
            .ForMember(
                dest => dest.FilePath,
                opt => opt.MapFrom(src => src.FileManager != null
                    ? src.FileManager.FilePath
                    : null)
            );
        ...
        CreateMap<ParticipantStatusCreateDto, ParticipantStatus>();

        CreateMap<ParticipantStatusUpdateDto, ParticipantStatus>();
    }
}